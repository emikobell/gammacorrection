...
accepted = ['h' 'b' 'j' 'n' 'm' 'q'];
    ...
    while isempty(character) | ~ismember(character, accepted) | length(character) > 1 % The first argument avoids NaN characters when non-ASCII characters are used, the second checks for approved letters, and the last avoids modifier keys (e.g. enter, space). No guarantee if the array is scalar, so the | argument is used.
